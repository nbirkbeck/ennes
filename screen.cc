...
bool IsEdgeBackground(const nacb::Image8& im, int x, int y,
  ...
  for (int k = 0; k < 8; ++k) {
    ...
    for (int c = 0; c < 3; ++c) {
      ...
      num_c_eq += (im.get(x + (coord == 0 ? high : k),
                          y + (coord == 1 ? high : k), c) == bg_color[c]);
    }
...
GetTopBlocks(const std::vector<std::pair<int, int>>& group,
             const std::unordered_map<int, int>& block_to_group) {
  std::vector<std::pair<int, int>> top_blocks;
  if (group.size() == 0)
    return top_blocks;
  const int group_index =
      block_to_group.find(group[0].second * kGridWidth + group[0].first)
          ->second;
...
bool IsBlockWalkable(const std::pair<int, int>& block, int x0,
                     nacb::Image8* image, const nacb::Imagef& g2,
                     const Color3b& bg) {
  int by = block.second;
  int bx = block.first;
  if (by == 0)
  ...
  for (int x = 0; x < 8; ++x) {
    ...
    double g =
        g2(bx * 8 + x + x0, by * 8, 1) + g2(bx * 8 + x + x0, by * 8 - 1, 1);
    num_grad += (g > 40);
  }
  if (num_grad >= 6) {
    int num_set = 0;
...
bool IsBlockListWalkable(const std::vector<std::pair<int, int>>& g,
  ...
  return (top_blocks.size() &&
          num_walkable >= 0.8 * top_blocks.size()) /*&& num_walkable >= 2)*/;
}

std::vector<BackgroundGroup>
FindBackgroundGroups(nacb::Image8* image, const Color3b& bg,
                     const std::map<int, int>& line_starts) {
  nacb::Imagef grads[3] = {nacb::Imagef(1, 1, 1), nacb::Imagef(1, 1, 1),
                           nacb::Imagef(1, 1, 1)};
  for (int c = 0; c < 3; ++c) {
  ...
  nacb::Imagef g2 =
      (grads[0] * grads[0]) + (grads[1] * grads[1]) + (grads[2] * grads[2]);
  ...
  for (int its = 0; its < 2; ++its) {
    for (int by = 0; by < kNesBlocksHeight; by++) {
      for (int bx = 0; bx < kGridWidth; bx++) {
        ...
        if (mask(bx, by) == 1 &&
            (its == 1 ||
             IsBlockWalkable(std::make_pair(bx, by), x0, image, g2, bg))) {
          ...
          while (!q.empty()) {
            ...
            block_to_group[block.second * kGridWidth + block.first] =
                groups.size();

            const std::set<int>& cur_colors =
                block_colors[block.second * kGridWidth + block.first];
            ...
            for (int k = 0; k < 4; ++k) {
              ...
              if (IsEdgeBackground(*image, block.first * 8 + x0,
                                   block.second * 8, bg, neigh[k][0],
                                   neigh[k][1])) {
                continue;
              }
              x0 = line_starts.lower_bound(ny * 8)->second;
              if (IsEdgeBackground(*image, nx * 8 + x0, ny * 8, bg,
                                   -neigh[k][0], -neigh[k][1])) {
                continue;
              }

              const std::set<int>& neigh_colors =
                  block_colors[ny * kGridWidth + nx];
              const int num_shared1 =
                  NumOverlappingColors(colors, neigh_colors);
              const int num_shared2 =
                  NumOverlappingColors(cur_colors, neigh_colors);
              ...
              if (num_shared1 >= 3 || num_shared2 >= 3 ||
                  ...
                  (its == 0 && IsBlockWalkable(std::make_pair(nx, ny), x0,
                                               image, g2, bg))) {
                mask(nx, ny) = 2;
          ...
          if (its == 0 && !IsBlockListWalkable(group, line_starts, image, g2,
                                               block_to_group, bg)) {
  ...
  for (const auto& g : groups) {
    ...
    const bool is_walkable =
        IsBlockListWalkable(g, line_starts, image, g2, block_to_group, bg);
...
void SegmentIntoCubes(const BackgroundGroup& group,
  ...
  for (auto p : pos) {
    ...
    if ((block.first + 1 < kGridWidth) &&
      ...
      int neigh[3] = {(block.second + 1) * kGridWidth + block.first,
