...
namespace {
bool SharesCommonBoundary(const nacb::Image8& im1, const nacb::Image8& im2,
                          const nacb::Vec3<int>& bg_color, int x1, int y1,
                          int x2, int y2) {
  ...
  for (int k = 0; k < 8; ++k) {
    ...
    for (int c = 0; c < 3; ++c) {
      ...
      // float d = float(ims[!high]->get(coord == 0 ? 7 : k, coord == 1 ? 7 : k,
      // c)) -
    ...
    // num_good += (sqrt(d2) <= 1);
    // num_bad += ims[!high]->get(coord == 0 ? 7 : k, coord == 1 ? 7 : k, 3) ==
    // 0;
  }
...
void GetSpriteImages(const Sprite* sprites,
  ...
  for (int i = 0; i < kNumSprites; ++i) {
    if (sprites[i].IsActive()) {
      ...
      DrawPattern(
          sprite_image, 0, 0, render_state.sprite_palette(),
          render_state.pattern_table(render_state.ppu().sprite_pattern_table()),
          sprites[i].pattern, sprites[i].HighColorBits(),
          sprites[i].GetTransformFlags());
...
FindConnectedSprites(const Sprite* sprites, const nacb::Vec3<int>& bg_color,
  ...
  for (int i = 0; i < kNumSprites; ++i) {
    ...
    if (!sprites[i].IsActive())
      continue;
    if (sprite_group_index.count(index))
      continue;
    ...
    while (!q.empty()) {
      ...
      for (int j = 0; j < 4; ++j) {
        ...
        if (sprite_pos.count(neigh_index) &&
            ...
            SharesCommonBoundary(
                sprite_images[sprite_pos.find(index)->second],
                sprite_images[sprite_pos.find(neigh_index)->second], bg_color,
                x, y, nx, ny)) {
          q.push(neigh_index);
    ...
    for (int j : sprite_groups[i]) {
      for (int y = 0; y < sprite_images[j].h; ++y) {
        for (int x = 0; x < sprite_images[j].w; ++x) {
          for (int c = 0; c < 3; ++c) {
            group_sprite(x + sprites[j].x - min_x, y + sprites[j].y() - min_y,
                         c) = sprite_images[j](x, y, c);
          }
        }
      }
    }
