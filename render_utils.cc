...
void RenderSprites(const nes::RenderState& render_state, int x_offset,
  ...
  for (int i = 0; i < kNumSprites; ++i) {
    if (sprites[i].IsActive() && (sprites[i].IsForeground() == foreground)) {
      DrawPattern(
          image, sprites[i].x + x_offset, sprites[i].y(),
...
RenderBackground(const nes::RenderSequence::FrameState& frame_state,
  ...
  for (int y = 0; y < kNesHeight; y += 8) {
    ...
    if (line_starts.empty() ||
        line_starts.lower_bound(y)->second != line_start) {
    ...
    // Currently only supports horizontal scrolling
    {
      ...
      for (int x = 8 * (current_state->ppu().vscroll() / 8); x < kNesWidth;
        ...
        DrawPattern(image, x - current_state->ppu().vscroll() + x_offset, y,
                    ...
                    render_state.pattern_table(
                        current_state->ppu().screen_pattern_table()),
                    name_table[(y >> 3) * (kNesWidth >> 3) + (x >> 3)],
                    high_color_bits);
      }
    }
    {
      ...
      for (int x = 0; x <= 8 * (current_state->ppu().vscroll() / 8); x += 8) {
        const int high_color_bits = GetAttributeColor(name_table, x, y);
        DrawPattern(image,
                    (x - current_state->ppu().vscroll()) + kNesWidth + x_offset,
                    y, render_state.image_palette(),
                    render_state.pattern_table(
                        current_state->ppu().screen_pattern_table()),
                    name_table[(y >> 3) * (kNesWidth >> 3) + (x >> 3)],
                    high_color_bits);
...
bool LoadRenderSequence(const std::string& filename, nes::RenderSequence* seq) {
